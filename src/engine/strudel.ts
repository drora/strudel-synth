  ...
  const res = await initStrudel({
    prebake: async () => {
      ...
      await Promise.all([
        ...
        samples(`${DRUM_CDN}/piano.json`, `${DRUM_CDN}/piano/`, { prebake: true }),
        samples(`${DRUM_CDN}/vcsl.json`, `${DRUM_CDN}/VCSL/`, { prebake: true }),
        ...
        samples(`${DRUM_CDN}/uzu-wavetables.json`, `${DRUM_CDN}/uzu-wavetables/`, { prebake: true }),
      ])
      try {
