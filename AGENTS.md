...
- **Code** — sheet/overlay inside Jam (`JamCodeSheet` → `TrackCodePane`). Kit-row `</> Code` opens one editor for every track (Save evaluates); header GitHub mark opens `drora/strudel-synth` in a new tab. Header **Export** / **Import** (right of Learn) reuse that all-tracks editor: Export saves `kit-root-scale-bpm.strudel`; Import loads a file or paste (invalid syntax is marked). Header **Share** captures the live mix (tracks + pads) for up to 3:00 (auto-stop); not Rec. Chip long-press / Edit in Code still opens one track. Last-touched chip: thin accent ring + tiny purple dot (`JamTrackChip`).
