...
- **Code** — sheet/overlay inside Jam (`JamCodeSheet` → `TrackCodePane`). Kit-row `</> Code` opens one editor for every track; header GitHub mark opens `drora/strudel-synth` in a new tab. Chip long-press / Edit in Code still opens one track. Last-touched chip: thin accent ring + tiny purple dot (`JamTrackChip`).
- **+ Track** — under Sounds/FX chips; pick role → `ROLE_PRESETS` default → Sound|FX sheet (no kit switch).
- **Liveloop** — cycle phase on the BPM ring (`JamPhaseRing`) and a subtle per-track phase tick on chips (`JamTrackChip` / `useLoopPhase` via `liveUpdateEngine`); mic Rec (`JamMicRec` / `engine/mic-sample.ts` — release the mic when Rec finishes; skip `initEngine` if Play is on or SuperDough is already up; never silent-unlock a live context); simple A/B punch (`JamABToggle` under Scale). Jam top chrome (Kit+Code, Root/Scale+A/B, walk) is anchored; pinch-zoom is off. Mute is 1-tap via the **M** on each track chip (Mix tab volume still available).
