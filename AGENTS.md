...
- **Liveloop** — cycle phase on the BPM ring (`JamPhaseRing`) and a subtle per-track phase tick on chips (`JamTrackChip` / `useLoopPhase` via `liveUpdateEngine`); mic Rec (`JamMicRec` / `engine/mic-sample.ts` — release the mic when Rec finishes; skip `initEngine` if Play is on or SuperDough is already up; never silent-unlock a live context); simple A/B punch (`JamABToggle` on the walk-hint row). Walk chips highlight the current center while playing (`cycleInt % walk.length`); **−/n/4/+** stepper beside chips sets walk length (melodic reshuffle, drums stay 1-cycle; not Bars / cat() tiling). Dice re-rolls root, scale, and walk length ∈ {2,3,4} (never 1 — length 1 is explicit via − only) and reshuffles unlocked melodic lanes to match. Jam top chrome (Kit+Code, Root/Scale, walk+A/B) is anchored. Pinch-zoom blocked where the browser allows. Mute is 1-tap via the **M** on each track chip (Mix tab volume still available).
...
| Mutate (patterns) | `engine/mutate.ts` + `engine/intensity.ts` — deterministic s()/note() transforms (Sparse…Stutter + Rotate/Reverse/…). Track via `lastTouchedTrackId`; Half/Double-time = song. Intensity is the footer stepper (1 as-is · 2 hats fill gaps (incl. `hh(3,8)` / `.euclid`) · 3 pad → arp → keys → fx · 4 bd ~ ~ ~ → bd ~ bd ~ · *N→*2N · perc snares (snare-family includes rim/rs/clap); bass+keys walk @0.5; Intensity snaps are partial overlays (L1 kit base · L2 hats only · L3 spawn only · L4 densify only); compose on play; edit@N owns+invalidates above; L3→4 pad carry, L4-only pad stays on 4; no mush FX; kit/Shuffle resets to 1). Undo snapshots (batch for song). **Mutate ≠ Shuffle ≠ Spice** |
...
- **Song seed (chord walk)** — melodic generate follows a `SongSeed` walk (`engine/song-seed.ts`): Song **Shuffle** rolls a new seed (same Root/Scale); per-track Shuffle keeps the seed and re-voices that lane; Sound still pins. Seed also carries `kickClock`, a per-role density `budget`, and a `mix` card (gain/pan/room/delay + bass/kick LPF + optional swing/euclid/duck feel). Song scales: minor/major/dorian/pentatonic/mixolydian/phrygian/lydian/harmonic_minor. Jam shows **concert-name walk chips** under Root/Scale (`walkConcertNames`: Cm · Ab · Eb · G) that highlight the current center while playing; **−/n/4/+** beside chips sets walk length (sticky on Song Shuffle; dice rolls N∈{2,3,4} never 1, regenerates melodic). Hats fill around the kick; amen/breaks hats use that voice (never leftover `hh`). Shuffle pin rewrites only the primary `s()` hit. `applyKit` is one generate (no double reshuffle). Each visit’s first generate (empty session) randomizes home + scale from the full `SONG_ROOTS` × `SONG_SCALES` pools (avoid current) and the Pads voice from `improvSoundChoices` (skip Rec). `hasPickedKit` is only for the picker. Later kit changes (picker / New kit) keep that pair. Dice next to Scale re-rolls both and remaps unlocked `note()` lanes.
