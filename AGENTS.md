...
- **Code** — sheet/overlay inside Jam (`JamCodeSheet` → `TrackCodePane`). Kit-row `</> Code` opens one editor for every track (Save evaluates); header GitHub mark opens `drora/strudel-synth` in a new tab. Header **Export** / **Import** / **Take** (right of Learn): Export writes `kit-root-scale-bpm.strudel` with a leading `// @jam` so Import restores kit/root/scale/tempo (code-only; intensity → 1; no note remap / no applyKit); Import file or paste marks invalid syntax. **Take** captures the live mix up to 3:00; ends on Stop / 3:00 / Take tap, and also on Shuffle / New kit; tap starts Play if stopped; not Rec. Chip long-press / Edit in Code still opens one track. Last-touched chip: thin accent ring + tiny purple dot (`JamTrackChip`).
