...
- **Liveloop** — cycle phase on the BPM ring (`JamPhaseRing`) and a subtle per-track phase tick on chips (`JamTrackChip` / `useLoopPhase` via `liveUpdateEngine`); mic Rec (`JamMicRec` / `engine/mic-sample.ts` — release the mic when Rec finishes; skip `initEngine` if Play is on or SuperDough is already up; never silent-unlock a live context); simple A/B punch (`JamABToggle` on the walk-hint row). Walk chips highlight the current center while playing (`cycleInt % walk.length`); row **1–4** sets walk length (melodic reshuffle, drums stay 1-cycle; not Bars / cat() tiling). Dice re-rolls root, scale, and walk length. Jam top chrome (Kit+Code, Root/Scale, walk+A/B) is anchored. Pinch-zoom blocked where the browser allows. Mute is 1-tap via the **M** on each track chip (Mix tab volume still available).
...
- **Song seed (chord walk)** — melodic generate follows a `SongSeed` walk (`engine/song-seed.ts`): Song **Shuffle** rolls a new seed (same Root/Scale); per-track Shuffle keeps the seed and re-voices that lane; Sound still pins. Seed also carries `kickClock`, a per-role density `budget`, and a `mix` card (gain/pan/room/delay + bass/kick LPF + optional swing/euclid/duck feel). Song scales: minor/major/dorian/pentatonic/mixolydian/phrygian/lydian/harmonic_minor. Jam shows **concert-name walk chips** under Root/Scale (`walkConcertNames`: Cm · Ab · Eb · G) that highlight the current center while playing; **1–4** on that row sets walk length (sticky on Song Shuffle; dice rolls a new N). Hats fill around the kick; amen/breaks hats use that voice (never leftover `hh`). Shuffle pin rewrites only the primary `s()` hit. `applyKit` is one generate (no double reshuffle). Each visit’s first generate (empty session) randomizes home + scale from the full `SONG_ROOTS` × `SONG_SCALES` pools (avoid current) and the Pads voice from `improvSoundChoices` (skip Rec). `hasPickedKit` is only for the picker. Later kit changes (picker / New kit) keep that pair. Dice next to Scale re-rolls both and remaps unlocked `note()` lanes.
