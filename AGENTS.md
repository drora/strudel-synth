...
- **GM soundfonts** — `@strudel/soundfonts` `registerSoundfonts()` in `engine/strudel.ts` prebake; Sound sheets carry the useful GM set (role-grouped; novelty like gunshot/helicopter excluded from melodic; orchestra_hit/timpani/fx_* on FX only) plus curated VCSL/Dirt orphans; `gm_*` voice-profile defaults held@0.85 with wash overrides for strings/pads/choir/organs.
