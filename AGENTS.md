...
| Mutate (patterns) | `engine/mutate.ts` + `engine/intensity.ts` — deterministic s()/note() transforms (Sparse…Stutter + Rotate/Reverse/…). Track via `lastTouchedTrackId`; Half/Double-time = song. Intensity is the footer stepper (1 as-is · 2 hats fill gaps · 3 pad → arp → keys → fx · 4 bd ~ ~ ~ → bd ~ bd ~ · *N→*2N · perc snares; bass walk @0.5; no mush FX; kit/Shuffle resets to 1). Undo snapshots (batch for song). **Mutate ≠ Shuffle ≠ Spice** |
