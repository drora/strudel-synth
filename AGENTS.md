...
- **Song seed (chord walk)** — melodic generate follows a `SongSeed` walk (`engine/song-seed.ts`): Song **Shuffle** rolls a new seed (same Root/Scale); per-track Shuffle keeps the seed and re-voices that lane; Sound still pins. Seed also carries `kickClock`, a per-role density `budget`, and a `mix` card (gain/pan/room/delay + bass/kick LPF + optional swing/euclid/duck feel). Song scales: minor/major/dorian/pentatonic/mixolydian/phrygian/lydian/harmonic_minor. Jam shows **concert-name walk chips** under Root/Scale (`walkConcertNames`: Cm · Ab · Eb · G) that highlight the current center while playing; walk stepper shows **N** (+ off at 4 / − off at 1); dice/New kit/Shuffle randomize 2–4 (never 1; Shuffle keeps hold 1); `rollSeed` without walkLength picks N uniformly then a legal pattern (no pinLen on applyKit). Hats fill around the kick; amen/breaks hats use that voice (never leftover `hh`). Shuffle pin rewrites only the primary `s()` hit. `applyKit` is one generate (no double reshuffle). Each visit’s first generate (empty session) randomizes home + scale from the full `SONG_ROOTS` × `SONG_SCALES` pools (avoid current) and the Pads voice from `improvSoundChoices` (skip Rec). `hasPickedKit` is only for the picker. Later kit changes (picker / New kit) keep that pair. Dice next to Scale re-rolls both and remaps unlocked `note()` lanes.
