...
- **Jam** (`JamShell`) — default UI; `freshStartJam()` on mount / bfcache `pageshow` (prefer last `kitId`, then `reshuffleUnlocked`). Song **Root/Scale** near Kit (defaults from kit `shuffle.root`/`scale`); feeds Shuffle; changing remaps melodic `note(...)` (drums/fx ignore). Melodic tracks expose **Octave** on the Mix tab (± shifts pitches; Sound pinned). Per-track **Lock** pins lane from Shuffle / Shuffle this / Mutate; Mix tab + chip; not kit Lock. Footer **Mutate** sheet applies deterministic pattern transforms (last-touched track; Half/Double-time = song; Undo).
...
src/
  ...
  components/
    jam/                  → JamShell, JamTrackChip, JamCodeSheet, JamTrackSheet, JamMutateSheet, kits UI,
...
- **Song seed (chord walk)** — melodic generate follows a `SongSeed` walk (`engine/song-seed.ts`): Song **Shuffle** rolls a new seed (same Root); per-track Shuffle keeps the seed and re-voices that lane; Sound still pins. Seed also carries `kickClock`, a per-role density `budget`, and a `mix` card (gain/pan/room/delay + bass/kick LPF + optional swing/euclid/duck feel). Song scales: minor/major/dorian/pentatonic/mixolydian/phrygian/lydian/harmonic_minor. Hats fill around the kick; amen/breaks hats use that voice (never leftover `hh`). Shuffle pin rewrites only the primary `s()` hit. `applyKit` is one generate (no double reshuffle). Footer **New kit** may randomize home from `SONG_ROOTS`; kit picker keeps the kit root.
