...
- **Jam** (`JamShell`) — default UI; `freshStartJam()` on mount / bfcache `pageshow` (prefer last `kitId`, then `reshuffleUnlocked`). Song **Root/Scale** near Kit (defaults from kit `shuffle.root`/`scale`); feeds Shuffle; changing remaps melodic `note(...)` (drums/fx ignore). Melodic tracks expose **Octave** on the Mix tab (± shifts pitches; Sound pinned). Per-track **Lock** pins lane from Shuffle / Shuffle this / Mutate; Mix tab + chip; not kit Lock. Footer **Mutate** sheet applies deterministic pattern transforms (last-touched track; Half/Double-time = song once-only (songTimeFeel normal|half|double; opposite restores); Undo). Footer **Intensity** stepper (− n/4 +; − off at 1, + off at 4). 1 as-is · 2 hats fill gaps (incl. `hh(3,8)` / `.euclid`) · 3 pad → arp → keys → fx · 4 bd ~ ~ ~ → bd ~ bd ~ · *N→*2N · kick densify (skip snare/rim/clap backbeat); bass+keys walk @0.5; Intensity snaps are partial overlays (L1 kit base · L2 hats only · L3 spawn + hat mute/volume memory · L4 densify + bass spawn if missing); compose on play; edit@N owns+invalidates above; L3→4 pad carry, L4-only pad stays on 4; L3 hats mute on first 2→3 (edits remembered; 4 restores prior bed; 3→2 prior); no mush FX; kit/Shuffle resets to 1. A/B slots stash codes + Root/Scale/walk and restore them on punch. Footer: 3-col grid ([Play/Pause | Stop] / Mic (🎤 mic emblem; not Rec / not red record-dot) / Mutate over Shuffle / Spice) with a vertical Intensity stepper on the right (+ n/4 −). **New kit** sits in kit chrome beside the kit name (not in the transport row). Pause = hush + keep cycle; Stop (■ / Ctrl+.) = hush + reset to 0:00. **Pads** sits under `+ Track` (8 fixed pads; SuperDough one-shots (~20ms warm lead) — jam stack stays put; hold-to-sustain + Vel; volume + Gain first (always open), then accordion Filter (LPF/HPF) / Envelope (ADSR) / Reverb (Room size) / Delay (Time, wet to 2) — one group open; same FxAccordion on track FX tab; Keep → last phrase only (from first note; sentence gap = one walk at BPM so hit/pause/hits spans walk N; octave change does not split; idle gap clears Keep · N), sheet closes, `@` holds (0.25 / 0.5 / 1–N×4; not 0.12), `~` gaps, `.velocity`; Rec holds smear at hold÷take, no chip; Keep rest-pads a short sentence to the walk (N×4 beats so hit timing stays and silence fills; no mid-walk tile) and `.slow(N)` when spanning the walk; Pads chip Sound sheet uses the live pad voice list, not vox Mouth/Yeah/Hmm/Auto; Clear drops the take; last voice / octave / mix / FX survive close; voice re-rolls once on hard reload then sticks).
- **Code** — sheet/overlay inside Jam (`JamCodeSheet` → `TrackCodePane`). Kit-row `</> Code` opens one editor for every track (Save flushes the editor into the store then evaluates; while stopped that starts Play of the flushed code); live all-tracks typing persists parseable `// @track` diffs even when the import-check banner fires (check is paste/file only); header GitHub mark opens `drora/strudel-synth` in a new tab. Header **Import** / **Export** / **Take** (right of Learn): Export writes `kit-root-scale-bpm.strudel` with a leading `// @jam` so Import restores kit/root/scale/tempo (code-only; intensity → 1; no note remap / no applyKit); **Import** opens the all-code paste sheet (Load file inside) — foreign `// @track` ids replace the jam; matching ids only patch codes; invalid syntax still banners. **Take** (red record-dot emblem) captures the live mix up to 3:00; ends on Stop / 3:00 / Take tap, and also on Shuffle / New kit; tap starts Play if stopped; not Rec (mic). Chip long-press / Edit in Code still opens one track. Last-touched chip: thin accent ring + tiny purple dot (`JamTrackChip`).
- **+ Track** — under Sounds/FX chips; pick role → `ROLE_PRESETS` default → Sound|FX sheet (no kit switch).
- **Play / live eval** — Play/Pause toggles (pause keeps cycle; Stop resets to 0:00). First Play and live updates silence unbalanced (or empty) lanes and peek the error (`Play · <name> skipped — syntax`); the rest of the jam still starts (does not fail the whole jam). Concurrent Save/Play while starting coalesce onto one `startPlayback` (no double CDN prebake). Save/`startPlayback` `{ ok: false }` peeks (`Code ·` / `Play ·`) and does not wipe track errors.
- **Liveloop** — cycle phase on the BPM ring (`JamPhaseRing`) and a subtle per-track phase tick on chips (`JamTrackChip` / `useLoopPhase` via `liveUpdateEngine`); Mic (`JamMicRec` / `engine/mic-sample.ts` — release the mic when Rec finishes; skip `initEngine` if Play is on or SuperDough is already up; never silent-unlock a live context); simple A/B punch (`JamABToggle` on the walk-hint row). Walk chips highlight the current center while playing (`cycleInt % walk.length`); walk control shows **N** (tiny “Walk” caption; + off at 4 / − off at 1). Dice / New kit / Shuffle randomize N ∈ {2,3,4} (never 1); Shuffle keeps 1 if user set hold. Dice also re-rolls root/scale and reshuffles unlocked melodic lanes. Jam top chrome (Kit+Code, Root/Scale, walk+A/B) is anchored. Pinch-zoom blocked where the browser allows. Mute is 1-tap via the **M** on each track chip (Mix tab volume still available).
...
src/
  ...
  components/
    ...
    transport/            → PlayButton, StopButton, TransportControls, SampleLoadingIndicator
...
- **GM soundfonts** — live-register via `engine/gm-soundfonts.ts` (`registerLiveGmSoundfonts` onto `globalThis.registerSound`, not package `registerSoundfonts` which can hit a dead map when `@strudel/web` dist embeds SuperDough). Priority-warm trio/family anchors in prebake; remainder warms in background so first stack haps are not discarded. Sound sheets: useful GM set (role-grouped; novelty excluded; orchestra_hit/timpani/fx_* + FX-flavored WTs crickets/curses/bad_day/echoes on FX only) plus curated VCSL/Dirt orphans; `gm_*` voice-profile: held@0.85 default; quiet wash (pads/strings/choir/fiddle) @1.15; organ wash @0.7; quiet wash winds/saxes/reed drones (flute/clarinet/…) @1.45; quiet held (nylon/mallets/plucks/…) @1.45; loud held (brass/slap/dist/hit) @0.6; VCSL pipeorgan_quiet/vibraphone_bowed wash; didgeridoo (FX sheet) wash; Dirt fm/sitar wash (sid short). Audio regress smoke (`npm run test:audio-regress`) covers GM live-register + onTrigger hits + Punch 909 generate (catalog lossless alone is not enough).
