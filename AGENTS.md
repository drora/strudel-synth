...
- **GM soundfonts** — live-register via `engine/gm-soundfonts.ts` (`registerLiveGmSoundfonts` onto `globalThis.registerSound`, not package `registerSoundfonts` which can hit a dead map when `@strudel/web` dist embeds SuperDough). Priority-warm trio/family anchors in prebake; remainder warms in background so first stack haps are not discarded. Sound sheets: useful GM set (role-grouped; novelty excluded; orchestra_hit/timpani/fx_* + FX-flavored WTs crickets/curses/bad_day/echoes on FX only) plus curated VCSL/Dirt orphans; `gm_*` voice-profile: held@0.85 default; quiet wash (pads/strings/choir) @1.15; organ wash @0.7; quiet held (flute/nylon/mallets/…) @1.45; loud held (brass/slap/dist/hit) @0.6; VCSL pipeorgan_quiet/vibraphone_bowed boosted; Dirt sid boosted.
