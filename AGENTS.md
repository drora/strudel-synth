...
- **GM soundfonts** — `@strudel/soundfonts` `registerSoundfonts()` in `engine/strudel.ts` prebake; Sound sheet: `gm_string_ensemble_1` (pad), `gm_violin` (lead), `gm_cello` (bass); wash profiles + auto-clip like other long samples.
