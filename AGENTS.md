...
| Mutate (patterns) | `engine/mutate.ts` + `engine/intensity.ts` — deterministic s()/note() transforms (Sparse…Stutter + Rotate/Reverse/…). Track via `lastTouchedTrackId`; Half/Double-time = song. Intensity is the footer stepper (1–4 this generate; pad add/remove at 3). Undo snapshots (batch for song). **Mutate ≠ Shuffle ≠ Spice** |
