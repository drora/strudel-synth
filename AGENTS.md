...
- **Code** — sheet/overlay inside Jam (`JamCodeSheet` → `TrackCodePane`). Kit-row `</> Code` opens one editor for every track (Save flushes the editor into the store then evaluates; while stopped that starts Play of the flushed code); live all-tracks typing persists parseable `// @track` diffs even when the import-check banner fires (check is paste/file only); header GitHub mark opens `drora/strudel-synth` in a new tab. Header **Import** / **Export** / **Take** (right of Learn): Export writes `kit-root-scale-bpm.strudel` with a leading `// @jam` so Import restores kit/root/scale/tempo/bars (code-only; intensity → 1; no note remap / no applyKit; missing header fields leave live chrome alone); Import file or paste marks invalid syntax. **Take** captures the live mix up to 3:00; ends on Stop / 3:00 / Take tap, and also on Shuffle / New kit; tap starts Play if stopped; not Rec. Chip long-press / Edit in Code still opens one track. Last-touched chip: thin accent ring + tiny purple dot (`JamTrackChip`).
...
| Song bars (1/2/4) | `engine/song-bars.ts` + `setSongBarsLength` in `jam-actions-a`; `// @jam bars=`; default 1; locked tile/trim via `cat`, unlocked Shuffle+fit; intensity → 1; Take stops |
