...
- `get_session` exposes `songRoot` / `songScale` / `songWalk` (centers + `walkLength` + `concertNames`). Walk chips highlight current; walk control shows N (+ off at 4 / − off at 1); dice/New kit/Shuffle randomize 2–4 never 1 (Shuffle keeps hold 1).
