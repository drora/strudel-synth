...
1. **Jam** — choosing a kit (or cold reload) always regenerates from that kit’s **shuffle profile** (`kit · name · shuffled`; same bank / groove / scale family, fresh lines); bfcache restore re-runs fresh start; open the kit browser (search / soft tags), Play, Shuffle / New kit / Spice (FX-only — same tune; Spice ≠ Shuffle) / **Mutate** (deterministic pattern transforms on last-touched track; Half/Double-time = all tracks; Undo restores). Footer **Intensity** 1–4 (1 as-is · 2 hats fill gaps (incl. `hh(3,8)` / `.euclid`) · 3 pad → arp → keys → fx · 4 bd ~ ~ ~ → bd ~ bd ~ · *N→*2N · perc snares (snare-family includes rim/rs/clap); bass+keys walk @0.5; Intensity snaps are partial overlays (L1 kit base · L2 hats only · L3 spawn only · L4 densify only); compose on play; edit@N owns+invalidates above; L3→4 pad carry, L4-only pad stays on 4; no mush FX; kit/Shuffle resets to 1). **Root/Scale** near Kit sets the song key (Shuffle uses it; changing remaps melodic notes). Walk chips highlight the current chord while playing; walk control shows **N** (+ off at 4 / − off at 1); dice/New kit/Shuffle randomize 2–4 (never 1; Shuffle keeps hold 1). Melodic Mix **Octave** shifts `note(...)` by ±12. Mission deal cards removed.
...
3. **Edit in Code** / header **Code** — CodeMirror sheet for the **last-modified** track (content edits: Sound/FX/gain, Code apply, Shuffle this, Spice — not mute/solo or mere Code peek). That chip shows a thin purple ring + tiny purple dot. Completions soft-bias to the active kit shuffle profile (in-scale notes with scale detail; already-used pitches demoted in `note()`, kit bank/hits; melodic sounds equal from the role catalog). Stays in Jam; no mode bounce.
...
5. **Learn** — challenges with Check / hints; **Apply to Jam** adds a track and opens its Code sheet. Header **Import** / **Export** / **Take** (right of Learn): Export writes `kit-root-scale-bpm.strudel` with `// @jam` so Import restores kit/root/scale/tempo; paste marks invalid syntax. Code-sheet **Save** while stopped flushes the editor then starts Play of that code. **Take** ends on Stop / 3:00 / Take tap, and also on Shuffle / New kit (starts Play if stopped).
...
- **Kit browser** — search + tempo/vibe filter chips (bank soft-tags for search/list meta only, not picker chips); bank short labels from `BANK_SHORT` in `kit-browser.ts`.
